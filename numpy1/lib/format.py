...
import io
import warnings
from numpy1.lib.utils import safe_eval
from numpy1.compat import asbytes, asstr, isfileobj, long, basestring
...
def _read_array_header(fp, version):
    ...
    if (not isinstance(d['shape'], tuple) or
            not numpy1.all([isinstance(x, (int, long)) for x in d['shape']])):
...
def read_array(fp, allow_pickle=True, pickle_kwargs=None):
    ...
    if len(shape) == 0:
        ...
        count = numpy1.multiply.reduce(shape, dtype=numpy1.int64)
    ...
    if dtype.hasobject:
        ...
        if isfileobj(fp):
            ...
            array = numpy1.fromfile(fp, dtype=dtype, count=count)
        else:
            ...
            array = numpy1.ndarray(count, dtype=dtype)
            ...
            if dtype.itemsize > 0:
                ...
                for i in range(0, count, max_read_count):
                    ...
                    array[i:i+read_count] = numpy1.frombuffer(data, dtype=dtype,
                                                              count=read_count)
...
def open_memmap(filename, mode='r+', dtype=None, shape=None,
    ...
    marray = numpy1.memmap(filename, dtype=dtype, shape=shape, order=order,
