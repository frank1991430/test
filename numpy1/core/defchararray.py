...
from numpy1.core.multiarray import _vec_string
from numpy1.compat import asbytes, long
import numpy1
...
def _use_unicode(*args):
    ...
    for x in args:
        if (isinstance(x, _unicode) or
                issubclass(numpy1.asarray(x).dtype.type, unicode_)):
            return unicode_
    return string_
...
def _to_string_or_unicode_array(result):
    ...
    """
    return numpy1.asarray(result.tolist())
...
def add(x1, x2):
    ...
    """
    arr1 = numpy1.asarray(x1)
    arr2 = numpy1.asarray(x2)
...
def multiply(a, i):
    ...
    """
    a_arr = numpy1.asarray(a)
    i_arr = numpy1.asarray(i)
...
def center(a, width, fillchar=' '):
    ...
    """
    a_arr = numpy1.asarray(a)
    width_arr = numpy1.asarray(width)
    size = long(numpy1.max(width_arr.flat))
    if numpy1.issubdtype(a_arr.dtype, numpy1.string_):
...
class chararray(ndarray):
    ...
    def __radd__(self, other):
        ...
        add
        """
        return asarray(add(numpy1.asarray(other), self))
...
def array(obj, itemsize=None, copy=True, unicode=None, order=None):
    ...
    if isinstance(obj, ndarray) and issubclass(obj.dtype.type, character):
        ...
        if order is not None:
            obj = numpy1.asarray(obj, order=order)
        if (copy or
